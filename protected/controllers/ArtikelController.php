		...
		if(isset($_POST['Artikel']))
		{
			...
			if (!file_exists($filename)) {
				if (!mkdir($filename, 0777, true)) {
					Yii::app()->user->setFlash('error', 'Gagal membuat direktori.');
				}
			}
			$foto=CUploadedFile::getInstance($model,'foto');
			$model->foto='foto-'.date('YmdHis').'.'.$foto->extensionName;
			if($model->save()){
				if(strlen(trim(CUploadedFile::getInstance($model,'foto'))) > 0){
					$foto->saveAs($filename . $model->foto);
				}
				Yii::app()->user->setFlash('success', '<strong>Success!</strong> Data berhasil diperbaharui.');
